...
namespace DG.Tweening
{
    ...
    public static class ShortcutExtensions
    {
        ...
        public static Tweener DOMoveXY(this Transform target, Vector3 endValue, float duration, bool snapping = false)
        {
            endValue.z = target.position.z;
            return DOTween.To(() => target.position, x => target.position = x, endValue, duration).SetOptions(snapping).SetTarget(target);
        }
    
        public static Tweener DOLocalMoveXY(this Transform target, Vector3 endValue, float duration, bool snapping = false)
        {
            endValue.z = target.localPosition.z;
            return DOTween.To(() => target.localPosition, x => target.localPosition = x, endValue, duration).SetOptions(snapping).SetTarget(target);
        }
